...
namespace TestFigur
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Mamy do dyspozycji nastepujące figury: ");
            foreach(FiguraPlaska f in figury)
            {
                Console.WriteLine(f.Opis());
                if(f is IPole)
                {
                    Console.WriteLine("Pole figury wynosi {0}", ((IPole)f).ObliczPole());
                }
                if (f is IObwod)
                {
                    Console.WriteLine("Pole figury wynosi {0}", ((IObwod)f).ObliczObwod());
                }
